...
def main() -> None:
    ...
    zeek_records = parse_zeek_log(Path(args.zeek_http_log))
    app_records = parse_json_lines(Path(args.app_log))
    attacker_session_records = parse_json_array(Path(args.session_log))
    ...
    if not normalized_logs:
        raise RuntimeError("No logs found to process.")
    ...
    generate_report(normalized_logs, per_session_analysis, Path(args.output_report))
    print(f"Report written to: {Path(args.output_report).resolve()}")
